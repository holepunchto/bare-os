 ...
 * @throws Thrown with the underlying system error code (for example `ENOENT`) when `dir` does not exist or cannot be entered.
 */
 ...
 * @param signal - Signal name or number to send (default `'SIGTERM'`); `0` probes for the process's existence without sending a signal.
 * @throws {UNKNOWN_SIGNAL} Thrown as an `OSError` when `signal` is a string that is not a recognized signal name.
 * @throws Thrown with the underlying system error code (for example `ESRCH`) when `pid` does not identify a running process.
 */
 ...
 * @param previous - A previous `CpuUsage` snapshot (from `threadCpuUsage()`) to compute a relative diff against.
 */
 ...
 * @throws {TITLE_OVERFLOW} Thrown as an `OSError` when the process title is 256 characters or longer.
 */
